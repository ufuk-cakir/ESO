...
class Gene:
    ...
    def _init_set_gene(self, band_position: int, band_height: int) -> None:
        ...
        assert self.min_position <= band_position, \
            f"min_position {self.min_position} should be less than or equal to band_position {band_position}"
        assert band_position < self.max_position, \
            f"band_position {band_position} should be less than max_position {self.max_position}"
        assert self.min_height <= band_height, \
            f"min_height {self.min_height} should be less than or equal to band_height {band_height}"
        assert band_height <= self.max_height, \
            f"band_height {band_height} should be less than or equal to max_height {self.max_height}"
        ...
        if (band_position + band_height) <= self.max_position:
            ...
            raise ValueError(f"The band position {band_position} and  band height {band_height} cannot exceed max_position {self.max_position}")
