...
class Preprocessing:
    ...
    def create_dataset(self, verbose, annotation_folder, sufix_file, file_names=None, augmentation=False):
        """
        ...
        for file in files.values:
            ...
            reader = AnnotationReader(self.species_folder,file, self.file_type, self.audio_extension
            )
    ...
    def repair_svl(self, file_names):
        ...
        for file in files.values : 
            ...
            df, sampleRate, start_m, end_m, audio_len, audio_seconds = reader.get_annotation_information_testing()
            ...
            for i in range(0, int(int(audio_len)/int(sampleRate)),4):
                ...
                for index, row in df.iterrows():
                    ...
                    labeled_end = int((row['frame']+row['duration'])/int(sampleRate))  
            ...
            df_repaired = pd.DataFrame({'frame': new_frames, 'value':new_values ,'duration': new_durations,
                                  'extent':new_extents,'label':new_labels})
    
            df_repaired = pd.concat([df_repaired,df[df['label']==self.positive_class]])
