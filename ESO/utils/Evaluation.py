...
class Evaluation:
    def __init__(
        ...
        __preprocessing_name = (
            "preprocessed" if apply_preprocessing else "unpreprocessed"
        )
    ...
    def _calc_amplitudes_to_predict(self, file_name, preprocessing, verbose=True):
        ...
        save_name = Path(
            self.save_amplitudes_path,
            file_name
            + "_amplitudes_to_predict_preprecoessing_"
            + str(self.apply_preprocessing_flag)
            + ".npy",
        )
    ...
    def _get_amplitudes_to_predict(self, file_name, preprocessing, verbose=True):
        ...
        if False:
            ...
            return self._calc_amplitudes_to_predict(
                file_name, preprocessing, verbose=verbose
            )
    ...
    def _process_one_file(self, file_name, model, preprocessing, verbose=True):
        amplitudes_to_predict, sample_rate, original_sample_rate, len_audio_amps = (
            self._get_amplitudes_to_predict(file_name, preprocessing, verbose=verbose)
        )
        if self.chromosome != None:
    ...
    def _normal_run(
        ...
        print_report=True,
        metric="f1",
        return_image_shape_and_parameters=True,
    ):
