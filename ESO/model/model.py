...
from eso.config import CHROMOSOME_CNN_ARCHITECTURE, CNN_ARCHITECTURE
from eso.model.cnn import BaseCNN
...
class Model:
    ...
    def evaluate(
        self, X_val, Y_val, metric=None, threshold=None, print_report=False
    ) -> tuple[float, str]:
        if metric is None:
            metric = self.metric
        ...
        if metric == "f1":
            return (f1, "F1")  # type:ignore
        elif metric == "accuracy":
            return (accuracy, "Accuracy")
        else:
            raise ValueError(f"Unknown metric {metric}. Please use 'f1' or 'accuracy'.")
