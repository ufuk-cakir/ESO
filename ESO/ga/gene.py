...
class Gene:
    """
    A class used to represent a Gene.

    The Gene is a representation of a band in a spectogram.
    The class has the option to randomly generate a band inside the range of
    min_position and max_position, with a given height between min_height and max_height.

    The spec_height is the height of the spectogram, and it is used to make sure that
    the band does not exceed the height of the spectogram.
    ...
    band_position : int
        Optional. Can be set to initialize a Gene with a given band position.
    band_height : int
        Optional. Can be set to initialize a Gene with a given band height.
    """
    ...
    def __init__(
        ...
        # Generate a gene with given band_position and band_height
            ...
            self._init_set_gene(band_position, band_height)  # type: ignore
    ...
    def _init_random_gene(self) -> None:
        ...
        rand_pos = random.randint(self.min_position, self.max_position - rand_height)
    ...
    def _init_random_pos_gene(self, band_height: int) -> None:
        ...
        """
        assert (
            self.min_height <= band_height
        ), "min_height must be less than or equal to band_height"
        assert (
            band_height <= self.max_height
        ), "band_height must be less than or equal to max_height"
    ...
    def set_band_position(self, new_band_position: int) -> None:
        ...
        """
        assert (
            self.min_position <= new_band_position
        ), "new_band_position should be greater than or equal to min_position"
        assert (
            new_band_position <= (self.max_position - self.band_height)
