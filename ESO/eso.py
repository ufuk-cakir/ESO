...
class ESO:
    ...
    def _train_baseline(self, full = False):
        ...
        self.minimum_input_shape = Model(
            input_shape=(1, image_shape[0], image_shape[1]),
            logger=self.logger,
    ...
    def optimize(self, max_generations, log_evolution=False):
        ...
        self.genetic_operator = GeneticOperator(**self.config.genetic_operator.dict())
    ...
    def evolve_population(self):
        ...
        if mutation_rate + crossover_rate + reproduction_rate != 1:
            raise ValueError(
        ...
        for _ in range(reproduction_size):
            ...
            self.evolution_logger.debug(f"Parent1 fitness: {parent1.get_fitness()}")
            self.evolution_logger.debug(f"Parent2 fitness: {parent2.get_fitness()}")
    ...
    def evaluate(self, save_name=None):
        ...
        f_baseline, confusion_matrix_baseline, baseline_params, baseline_image_shape, baseline_pixels = self._evaluate_model(model_type="baseline", test_type="normal")
        f_chromosome, confusion_matrix_chromosome, chromosome_params, chromosome_image_shape, chromosome_pixels= self._evaluate_model(model_type="chromosome", test_type="normal")
        ...
        df["PARAMS_improvement"] = (df["PARAMS"] - df["PARAMS"].shift(1)) / df["PARAMS"].shift(
            1
        )
        ...
        '''
        # save config to second sheet
        config = self.config.dict()
        config = {k: [v] for k, v in config.items()}
        config = pd.DataFrame(config)
        with pd.ExcelWriter(os.path.join(self.results_path, "evaluation.xlsx")) as writer:
            df.to_excel(writer, sheet_name="evaluation")
    ...
    def _retrain_full(self, num_epochs = 40, batch_size = 64, learning_rate = 0.001, force_retrain_baseline = True):
        model_args = {
            "optimizer_name": "adam",
        ...
        chromosome.train(chrom_train_X, chrom_train_Y, chrom_val_X, chrom_val_Y)
        chromosome.save_model(self.results_path,"eso_chromosome")
        chromosome.save_model(self.results_path)
    ...
    def _evaluate_model(self, model_type="baseline",test_type="normal"):
        if model_type == "baseline":
            ...
            model_path = os.path.join(self.results_path, "chromosome_cnn_state.pth")
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            model = Model.load_cnn(model_path, device)
